...
@export_range(0.0, 1000.0) var water_physics_speed: float = 80
...
@export var surface_color: Color = Color("f1f6f0")
@export var water_fill_color: Color = Color("f1f6f0")
...
func _initiate_water():
	...
	var new_collsionshape: CollisionShape2D = CollisionShape2D.new()
	var new_shape: RectangleShape2D = RectangleShape2D.new()
	new_shape.size = water_size
	new_collsionshape.shape = new_shape
	new_collsionshape.position = water_size/2.0 + Vector2(0, surface_pos_y/2.0)
	new_area.add_child(new_collsionshape)
